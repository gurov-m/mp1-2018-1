...
void Battle::SetAddress(int *deckAddress, int decks, int position)//записывает адреса кор. и вокруг них
{
	if (position == 1)//если корабль по вертикали
	{
		...
		for (int i = 0; i < decks + 2; i++)//обвод корабл€
		{
	...
	for (int a = 1; a <= decks; a++)//проверка каждой палубы 
	{
		if (random == 1)
		{
			for (int i = 0; i < 144; i++)
			{
				...
				{
					errors = 1;// только дл€ ручного ввода
					return 0;
				}
			}
			*(deckAddress += 12);
...
void Battle::EncircleDeadPlayerShip(int *firstdeck, int position, int decks)//обвод уитого корабл€
{
	...
	if (position == 1)
	{
		for (int i = 0, j = 12; i < decks; i++)
		{
			if (*(firstdeckAddress) == 3)
			{
				...
				hit = 1;
				if (hitsCount == 1)
					firstHittedDeck = &*(firstdeckAddress);  // присваивает адрес только первого попадени€
			}
...
void Battle::IfHittedPlayerShip(int*hit)//ѕоиск корабл€ в который попали
{
	if ((*(hit + 1) != 1) && (*(hit - 1) != 1) && (*(hit + 12) != 1) && (*(hit - 12) != 1) &&//дл€ однопалубных обвод в случае попадани€
...
// после этой функции использовать DrawPlayer и DrawPC
void Battle::IfHittedPCShip(int*hit)//ѕоиск корабл€ в который попали
{
